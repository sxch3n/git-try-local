...
class Hall:
    ...
    def handle_msg(self, player, msg):
        ...
        instructions = b'Instructions:\n'\
            ...
            + b'[<json> json_str] to send json data\n' \
        ...
        if "name:" in msg:
            ...
            if len(msg.split()) >=2:
                json_str = ''.join(msg.split()[1:])
                parsedStr = json.loads(json_str)
                print("parsed json string from "+ player.name +":\n")
                print(json.dumps(parsedStr,indent=4))
                if player.name in self.room_player_map:
                    self.rooms[self.room_player_map[player.name]].broadcast(player, ("json: " + json_str).encode())
